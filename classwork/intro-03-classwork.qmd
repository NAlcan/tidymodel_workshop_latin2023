---
title: "3 - What makes a model? - Classwork"
subtitle: "Introduction to tidymodels"
editor_options: 
  chunk_output_type: console
---

We recommend restarting R between each slide deck!

## Setup

Setup from deck 2

```{r}
library(tidymodels)

set.seed(123)
taxi_split <- initial_split(taxi, prop = 0.8, strata = tip)
taxi_train <- training(taxi_split)
taxi_test <- testing(taxi_split)
```

## Your turn

How do you fit a linear model in R?

How many different ways can you think of?

Discuss with your neighbor!

## To specify a model

Con el paquete parnship especificamos un `modelo` (tipo de modelo eg: regresion lineal), luego especificamos el `engine`, es decir, el algoritmo (paquete) qyue vamos a usar para estimar los parametros del modelo y luego el `modo` (en regresion el modo esta por defecto porque es regresion, por ejemplo en unn CART especificamos en el modo si es de clasificicación o regresion)

```{r}
# Model
linear_reg()

# Engine
linear_reg() %>%
  set_engine("glmnet")

# Mode - Some models have a default mode, others don't
decision_tree() %>% 
  set_mode("regression")
```

## Your turn

Edit the chunk below to use a logistic regression model.

Extension/Challenge: Edit this code to use a different model. For example, try using a conditional inference tree as implemented in the partykit package by changing the engine - or try an entirely different model type!

All available models are listed at <https://www.tidymodels.org/find/parsnip/>

```{r tree_spec}
tree_spec <- decision_tree() %>% 
  set_mode("classification")

tree_spec


lr_spec2 <- logistic_reg() %>%
  set_engine("glm", family = binomial(link = "logit"))

lr_spec2


rf_spec3 <- rand_forest() %>% 
  set_mode("classification") %>% 
  set_engine("randomForest")


rf_spec3

```

## A model workflow

```{r}
tree_spec <-
  decision_tree(cost_complexity = 0.002) %>% 
  set_mode("classification")
```

Fit parsnip specification:

```{r}
tree_spec %>% 
  fit(tip ~ ., data = taxi_train) 
```

Fit with a workflow:

```{r}
workflow() %>%
  add_formula(tip ~ .) %>%
  add_model(lr_spec2) %>%
  fit(data = taxi_train) 
```

"Shortcut" by specifying the preprocessor and model spec directly in the `workflow()` call:

```{r}
workflow(tip ~ ., tree_spec) %>% 
  fit(data = taxi_train) 
```

## Your turn

Edit the chunk below to make a workflow with your own model of choice!

Extension/Challenge: Other than formulas, what kinds of preprocessors are supported?

```{r tree_wflow}
tree_spec <-
  decision_tree(cost_complexity = 0.002) %>% 
  set_mode("classification")

tree_wflow <- workflow() %>%
  add_formula(tip ~ .) %>%
  add_model(tree_spec)

tree_wflow

rf_spec3

rf_wflow <- 
  workflow(tip ~.) %>% 
  add_model (rf_spec3) 


rf_fit <- 
  workflow(tip ~., rf_spec3) %>% 
  fit(data = taxi_train)

```

## Predict with your model

```{r}
tree_fit <-
  workflow(tip ~ ., tree_spec) %>% 
  fit(data = taxi_train) 

```

## Your turn

What do you get from running the following code? What do you notice about the structure of the result?

```{r}
predict(tree_fit, new_data = taxi_test)
predict(tree_fit, new_data = taxi_test, type = "prob")
```

## Your turn

What do you get from running the following code? How is `augment()` different from `predict()`?

```{r}
augment(tree_fit, new_data = taxi_test)

augment(tree_fit, new_data = taxi_test) %>% 
  select(starts_with(".pred"))


augment(rf_fit, new_data = taxi_test)


```

## Understand your model

```{r}
library(rpart.plot)

tree_fit %>%
  extract_fit_engine() %>%
  rpart.plot(roundint = FALSE)
```

## Your turn

Try extracting the model engine object from your fitted workflow!

```{r}
### Your code here

rf_fit %>%
  extract_fit_engine() %>%
  randomForest::varImpPlot()

rf_fit %>%
  extract_fit_engine() %>%
  randomForest::treesize()


```

What kind of object is it? What can you do with it?

⚠️ Never `predict()` with any extracted components!

You can also read the documentation for object extraction:
https://workflows.tidymodels.org/reference/extract-workflow.html

